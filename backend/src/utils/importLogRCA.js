...
/**
 * 将LogRCA分析结果导入到MongoDB
 */
async function importLogRCAData() {
  try {
    // 设置文件路径
    const filePath = path.resolve(__dirname, '../../../LogRCA/output/log_analysis_report.json');
    ...
    const formattedLogs = logEntries.map(logEntry => {
      ...
      return {
        ...
        summary: summary,
        vm_id: logEntry.vm_id, // 直接在文档根级别添加vm_id
        analysis: {
          summary: summary,
...
if (require.main === module) {
  importLogRCAData()
    .then(() => console.log('导入脚本完成'))
    .catch((err) => console.error('导入脚本失败:', err));
}
